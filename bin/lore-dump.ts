...
async function main({seed, count, graph, positionals}: {seed: number, count: number, graph: number, positionals: string[]}) {
  if (graph < 0) {
    console.log(ALL_GRAPHS.map((pg, i) => `${`${i}`.padStart(3)} ${pg.name}`).join("\n"));
    return;
  }
  ...
  console.log(pg.name);
  console.log(`states: \n  ${pg.states.map(st =>
    state[st] ? `+ \x1b[48;5;19m${st}\x1b[m` : `- ${st}`).join('\n  ')}`);
  const rng = new PseudorandomStream(seed);
  try {
    for (let i = 0; i < count; i++) {
      const text = pg.generate(rng, state, MOCK_FORMAT);
      const wrapped = wrap(text, {indent: '', width: 96}).split('\n').join('\n    ');
      console.log(`${i % 2 === 0 ? '\x1b[38;5;118m' : ''}${`${i}`.padStart(3)} ${wrapped}\x1b[m`);
    }
  } catch (e) {
