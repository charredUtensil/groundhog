...
import { CavernWithBaseplates, CavernWithBaseplatesAndPaths } from "../../models/cavern";

/**
 * Performs Delaunay triangulation over all the baseplates with the 'cave' kind to generate
 * ambiguous paths.
 */
export default function triangulate(cavern: CavernWithBaseplates): CavernWithBaseplatesAndPaths {
  ...
  for (let i = 0; i < delaunay.triangles.length; i++) {
    ...
    if (i > delaunay.halfedges[i]) {
      ...
      const dest = caveBaseplates[delaunay.triangles[i + ((i % 3 === 2) ? -2 : 1)]];
