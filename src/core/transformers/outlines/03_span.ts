 ...
 */
export default function span(cavern: CavernWithBaseplatesAndPaths): CavernWithBaseplatesAndPaths {
  const clusters: Map<number, number> = new Map();
  let nextCluster = 1;
  const paths = []

  for (const path of [...cavern.paths].sort((a, b) => a.batDistance - b.batDistance)) {
