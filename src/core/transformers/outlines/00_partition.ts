...
import { Cavern, CavernWithBaseplates } from "../../models/cavern";
...
class Partitioner {
  ...
  private split(bp: Baseplate) {
    if (bp.width > bp.height * 2) {
      // Cut vertically
      const x = this.rng.betaInt({a: 5, b: 5, min: bp.left + 1, max: bp.right});
      this.clone(bp, bp.id, {right: x});
      this.clone(bp, this.id(), {left: x});
    } else if (bp.height > bp.width * 2) {
      // Cut horizontally
      const y = this.rng.betaInt({a: 5, b: 5, min: bp.top + 1, max: bp.bottom});
...
export default function partition(cavern: Cavern): CavernWithBaseplates {
  const partitioner = new Partitioner(cavern.context)
  while (!partitioner.done) {
    partitioner.step()
    cavern.context.logger.info({...cavern, baseplates: partitioner.baseplates})
  }
