...
function comments(cavern: FencedCavern) {
  return (`Cavern generated by groundHog v{VERSION}
...
function pointSet(points: Point[], [ox, oy]: Point): string {
  return points.map(([x, y]) => `${(x + ox).toFixed()},${(y + oy).toFixed()}/`).join()
}
...
export function serializeHazards(hazards: Grid<Erosion | Landslide>, offset: Point): string {
  ...
  return Array.from(out.entries()).map(([key, points]) => (
    `${key}:${pointSet(points, offset)}`
  )).join('\n')
}
...
${grid(
  ...
  (x, y) => {
    ...
    const offset = (!tile.isWall && !cavern.discoveryZones.get(x, y)?.openOnSpawn) ? 100: 0
...
${
  ...
  `${'0,'.repeat(1 + cavern.right - cavern.left)}\n`
    .repeat(1 + cavern.bottom - cavern.top)
}}
resources{
crystals:
