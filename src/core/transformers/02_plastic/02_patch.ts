...
export default function patch(cavern: RoughPlasticCavern): RoughPlasticCavern {
  ...
  const queue: {x: number, y:number, facing: Cardinal4 | null}[] = tiles.flatMap((_, x, y) => [
    {x, y, facing: null},
  ...
  while (queue.length) {
    ...
    if (!visited.get(x, y) && (tiles.get(x, y)?.isWall ?? true)) {
      const neighbors = NSEW.map(
        f => ({x: x + f[0], y: y + f[1], facing: f}));
      const wallNeighbors = neighbors.filter(({x, y}) => tiles.get(x, y)?.isWall ?? true)
      ...
      const needsPatch = () => {
        if (wallNeighbors.length < 2) {
          ...
          if (a.x === b.x || a.y === b.y) {
            return discoveryZones.get(a.x, a.y) === discoveryZones.get(b.x, b.y);
          }
          return false;
        }
        return false;
