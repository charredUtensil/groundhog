...
type Sorted = {plan: Flooded & {architect?: Architect}, hops: number, index: number}
...
export default function establish(
  ...
  function doArchitect({plan, hops, index}: Sorted): Architected {
    ...
    const crystalRichness = curved(
      (plan.kind === 'cave' ? cavern.context.caveCrystalRichness: cavern.context.hallCrystalRichness), props)
