...
describe('caveNucleus', () => {
  ...
  it('for one square cave with even size is four tiles', () => {
    ...
    const grid = new LayerGrid()
    caveNucleus(grid, plan)
    expect(grid.atLayer(0)).toEqual([[9, 9], [9, 10], [10, 9], [10, 10]])
  })
})
...
describe('trail', () => {
  describe('with no baroqueness', () => {
    it('wraps a single tile', () => {
      const grid = new LayerGrid()
      grid.set(14, 26, 0)
      const r = trail(grid, {} as PseudorandomStream, 0, 1, {x: 14, y: 25, vx: 1, vy: 0})
      const expected = [[14, 25], [15, 26], [15, 27], [14, 27], [13, 26], [13, 25]]
    ...
    it('wraps a 2x2 square', () => {
      ...
      grid.set(8, 5, 0)
      grid.set(9, 5, 0)
      const r = trail(grid, {} as PseudorandomStream, 0, 1, {x: 8, y: 3, vx: 1, vy: 0})
      const expected = [
        [ 8, 3 ],
        [ 9, 3 ],
        [ 10, 4 ],
