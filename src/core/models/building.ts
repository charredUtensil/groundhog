...
class BuildingTemplate {
  ...
  atTile: (args: {
    x: number,
    y: number,
    facing: Cardinal4} & BuildingExtraArgs
  ) => Building = (args) => ({
    ...atCenterOfTile(args),
    template: this,
    foundation: rotateFootprint(this.footprint, args.facing)
      .map(([x, y]) => [x + args.x, y + args.y]),
    level: args.level ?? 1,
...
export const TOOL_STORE = new BuildingTemplate('BuildingToolStore_C', 'E', 3, 0, F_DEFAULT)
export const TELEPORT_PAD = new BuildingTemplate('BuildingTeleportPad_C', 'T', 2, 0, F_DEFAULT)
// The Docks faces the LAND tile with the water BEHIND it.
// Note the water tile is not counted as part of the foundation
export const DOCKS = new BuildingTemplate('BuildingDocks_C', 'D', 1, 1, F_DEFAULT)
// The Canteen is functionally symmetrical, but if you care, the end with
// the yellow/black chevron piece is the FRONT.
export const CANTEEN = new BuildingTemplate('BuildingCanteen_C', 'C', 1, 1, F_CANTEEN_REFINERY)
// Power Station origin is the RIGHT side of the building where miners put
// the crystals in.
export const POWER_STATION = new BuildingTemplate('BuildingPowerStation_C', 'P', 2, 2, F_POWER_STATION)
export const SUPPORT_STATION = new BuildingTemplate('BuildingSupportStation_C', 'S', 2, 3, F_DEFAULT)
export const UPGRADE_STATION = new BuildingTemplate('BuildingUpgradeStation_C', 'U', 3, 3, F_DEFAULT)
// Geological Center origin is the BACK of the building.
export const GEOLOGICAL_CENTER = new BuildingTemplate('BuildingGeologicalCenter_C', 'G', 5, 2, F_DEFAULT)
// Ore Refinery origin is the FRONT of the building were miners put ore in.
export const ORE_REFINERY = new BuildingTemplate('BuildingOreRefinery_C', 'R', 4, 3, F_CANTEEN_REFINERY)
export const MINING_LASER = new BuildingTemplate('BuildingMiningLaser_C', 'L', 1, 1, F_MINING_LASER)
// Super teleport origin is the LEFT side of the building when facing it.
export const SUPER_TELEPORT = new BuildingTemplate('BuildingSuperTeleport_C', 'X', 2, 4, F_SUPER_TELEPORT)
