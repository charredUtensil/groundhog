...
export function getFlags<T>(opts: Omit<NonNullable<Parameters<typeof parseArgs>[0]>, 'options'> & {usage: string, options: FlagsConfig<T>}) {
  const flags = Object.keys(opts.options) as (keyof T)[];
  function usage() {
    const flagsHelp = flags.sort().map(k => {
  ...
  try {
    const { values, positionals } = parseArgs({...opts, options: {help: HELP_FLAG, ...opts.options}}) as any;
    if (values.help) {
      usage();
      exit(0);
    }
