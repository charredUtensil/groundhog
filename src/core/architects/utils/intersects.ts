...
export function intersectsAny(plans: readonly FloodedPlan[], plan: FloodedPlan, fluid: FluidType) {
  return plans.some((p) => plan.intersects[p.id] && p.fluid === fluid)
}

export function intersectsOnly(plans: readonly FloodedPlan[], plan: FloodedPlan, fluid: FluidType) {
  return !plans.some((p) => plan.intersects[p.id] && p.fluid !== fluid)
}
