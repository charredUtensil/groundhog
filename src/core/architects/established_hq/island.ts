...
import { TOOL_STORE, POWER_STATION, SUPPORT_STATION, DOCKS, GEOLOGICAL_CENTER, MINING_LASER } from "../../models/building";
import { Plan } from "../../models/plan";
import { randomlyInTile } from "../../models/position";
import { Tile } from "../../models/tiles";
import { RAPID_RIDER } from "../../models/vehicle";
import { sprinkleCrystals, sprinkleOre } from "../utils/resources";
...
const T1_CRYSTALS = T1_BUILDINGS.reduce(
  (r, bt) => r + bt.crystals,
  0
);
...
export const ISLAND_BASE: Pick<
  Architect<HqMetadata>, "crystalsFromMetadata" | "prime" | "placeBuildings" | "placeCrystals" | "placeEntities" | "script"
> = {
  crystalsFromMetadata: (metadata) => RAPID_RIDER.crystals + metadata.crystalsInBuildings,
  prime: ({ cavern, plan }) => {
  ...
  placeEntities: ({cavern, plan, minerFactory, vehicleFactory}) => {
    ...
    const vehicles = [vehicleFactory.create({
      ...pos,
      planId: plan.id,
      driverId: miner.id,
