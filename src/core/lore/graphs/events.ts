...
export const FOUND_HQ = phraseGraph<State>(
  ({ pg, state, start, end, cut, skip }) => {
    ...
    pg(
      pg(positive_greeting, neutral_greeting)
        .then('Now, ', 'Now you should be able to'),
      neutral_greeting
        .then(state('hasMonsters').then(
      ...
      .then(
        pg(
          ...
          state('lostMinersTogether', 'lostMinersApart').then('find those lost Rock Raiders!')
        ).then(tail),
...
export const FOUND_LOST_MINERS = phraseGraph<FoundLostMinersState>(
  ({ pg, state, start, end, cut, skip }) => {
    start.then(
      ...
      'We need to find all ${lostMinersCount} before we can leave.',
    ).then(end)
});
